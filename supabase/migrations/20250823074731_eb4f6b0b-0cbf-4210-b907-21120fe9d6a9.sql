-- Update user role enum to include 'accounts'
ALTER TYPE user_role ADD VALUE 'accounts';

-- Update submission status enum to reflect new flow
...
-- Update submissions table to use new status enum
ALTER TABLE submissions ALTER COLUMN status TYPE submission_status_new USING status::text::submission_status_new;

-- Drop old enum
DROP TYPE IF EXISTS submission_status_old;

-- Update default status for new submissions
ALTER TABLE submissions ALTER COLUMN status SET DEFAULT 'Pending Admin Approval';
